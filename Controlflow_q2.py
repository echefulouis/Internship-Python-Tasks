...
win_count_dict = {}
names_won=[]
for win_year,list_names_won in winners.items():
    names_won+=list_names_won

for name in names_won:
    if name not in win_count_dict:
        win_count_dict[name]=names_won.count(name)
number=[]
for name,num in win_count_dict.items():
    number.append(num)
for name in win_count_dict:
    if win_count_dict[name]==max(number):
